...
import "./SafeMath.sol";
import "./BabyDogeLibrary.sol";

// library containing some math for dealing with the liquidity shares of a pair, e.g. computing their exact value
// in terms of the underlying tokens
library BabyDogeLiquidityMathLibrary {
  ...
  function getReservesAfterArbitrage(
    ...
    (reserveA, reserveB) = BabyDogeLibrary.getReserves(
      factory,
      tokenA,
      tokenB
    );
    ...
    require(
      ...
      "BabyDogeV2ArbitrageLibrary: ZERO_PAIR_RESERVES"
    );
    ...
    if (aToB) {
      uint256 amountOut = BabyDogeLibrary.getAmountOut(
        amountIn,
        reserveA,
        reserveB,
        997
  ...
  function getLiquidityValue(
    ...
    (uint256 reservesA, uint256 reservesB) = BabyDogeLibrary.getReserves(
      factory,
      tokenA,
      tokenB
    );
    IBabyDogePair pair = IBabyDogePair(
      BabyDogeLibrary.pairFor(factory, tokenA, tokenB)
    );
    bool feeOn = IBabyDogeFactory(factory).feeTo() != address(0);
